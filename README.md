...
### Detailed steps:
  ...
  - Go to AWS EC2 => instance => launch instance:
    a. Jenkins => AMI Ubuntu22(free tier) => t2.small => select key pair => Jenkins SG => Advanced settings => user data (<PATH>) => launch.
    b. Nexus => AMI CentOS 9(free tier) => t2.medium => select key pair => Nexus SG => Advanced settings => user data (<PATH>) => launch.
    c. Sonarqube => AMI Ubuntu22(free tier) => t2.medium => select key pair => Sonarqube SG => Advanced settings => user data (<PATH>) => launch.
  ...
  - Add Environment variable and Sonar Analysis stage in pipeline which is in <PATH>.
  ...
  - Add Quality gate stage in pipeline which is in <PATH>.
  ...
  - Add Upload Artifact stage in the pipeline which is in <PATH>.
